resource "azurerm_virtual_network_peering" "vnet_peering_to_hub" {
  for_each = { for vnet, val in local.vnets : vnet => val if val.type != "hub" && val.type != "plink" }
...
resource "azurerm_virtual_network_peering" "vnet_peering_to_spoke" {
  ...
  resource_group_name          = "rg-${element(keys({ for n, val in local.vnets : n => val if val.type == "hub" }), 0)}-${substr(each.value.location, 0, 1)}${local.resource_suffix}"
